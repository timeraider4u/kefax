...
public class Test0011_InFile {
	...
	public void checkParserResult() throws Exception {
		...
		Assert.assertEquals("out.o", PathCmd_5_Var
		.getPath());
		//5
		...
		Assert.assertEquals("abc.c", Argument_6_Var
		.getIn());
		//6
		...
		Assert.assertEquals("_BAR2", SimpleMacro_10_Var
		.getName());
		//10
		final CmdLine CmdLine_11_Var
		 = (CmdLine)Lines_0_list.get(2);
		Assert.assertNotNull(CmdLine_11_Var
		);
		final EList<? extends EObject> Arguments_11_list = CmdLine_11_Var
		.getArguments();
		Assert.assertNotNull(Arguments_11_list);
		Assert.assertEquals(3, Arguments_11_list.size());
		//11
		final Argument Argument_12_Var
		 = (Argument)Arguments_11_list.get(0);
		Assert.assertNotNull(Argument_12_Var
		);
		//12
		final SimpleMacro SimpleMacro_13_Var
		 = (SimpleMacro)Argument_12_Var
		.getMacro();
		Assert.assertNotNull(SimpleMacro_13_Var
		);
		Assert.assertEquals("FOO2", SimpleMacro_13_Var
		.getName());
		//13
		final Argument Argument_14_Var
		 = (Argument)Arguments_11_list.get(1);
		Assert.assertNotNull(Argument_14_Var
		);
		Assert.assertEquals("C:\\temp\\infile.c", Argument_14_Var
		.getIn());
		//14
		final Argument Argument_15_Var
		 = (Argument)Arguments_11_list.get(2);
		Assert.assertNotNull(Argument_15_Var
		);
		//15
		final SimpleMacro SimpleMacro_16_Var
		 = (SimpleMacro)Argument_15_Var
		.getMacro();
		Assert.assertNotNull(SimpleMacro_16_Var
		);
		Assert.assertEquals("FOO2", SimpleMacro_16_Var
		.getName());
	}
