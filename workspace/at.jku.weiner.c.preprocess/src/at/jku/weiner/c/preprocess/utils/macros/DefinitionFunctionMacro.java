...
public final class DefinitionFunctionMacro implements DefinitionMacro {
	...
	private enum ReplacementState {
		...
		Stringify,
		StringifyEnd,
		ConcatenateLookAheadA,
		ConcatenateLookAheadB,
		Concatenate1a,
		Concatenate1b,
		Concatenate2,
		ConcatenateEnd,
		ConcatenateAndEatComma,
	}
			...
			switch (state) {
				...
				case ConcatenateEnd:
					this.addConcatenReplacement(parenID, code, ranges, token,
							text, replace, temp, addTemporary);
					final int insertionIndex = ranges
		...
		switch (currState) {
			case Normal: {
				if (tokenType == InternalPreprocessLexer.RULE_HASH) {
					...
					final boolean lookAHead = this
							.lookAHead(i, currState, true);
	...
	private void addConcatenReplacement(final long parenID,
		...
		final int newType = next.getType();
		...
		} else if ((oldType == InternalPreprocessLexer.RULE_DECIMAL_LITERAL)
				&& (newType == InternalPreprocessLexer.RULE_ID)) {
			final String prevText = prev.getText();
			final String nextText = next.getText();
			code.remove(insertionIndex);
			ranges.undoInsertion();
			prev.setText(prevText + nextText);
			prev.setType(InternalPreprocessLexer.RULE_HEX_LITERAL);
		}
	}
}
